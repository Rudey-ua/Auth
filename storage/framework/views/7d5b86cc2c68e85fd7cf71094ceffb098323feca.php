<div class="mb-3">
    <label class="form-label">Порода</label>

    <select required style="width: 50%;height: 50px; font-size: 16px" name="bred" class="form-select">
        <option value="" selected disabled>Выбрать</option>
        <option value="Немецкий шпиц">Немецкий шпиц</option>
        <option value="Йоркширский терье">Йоркширский терье</option>
        <option value="Чихуахуа">Чихуахуа</option>
        <option value="Немецкая овчарка">Немецкая овчарка</option>
        <option value="Лабрадор-ретривер">Лабрадор-ретривер</option>
        <option value="Хаски">Хаски</option>
    </select>
</div>

<?php /**PATH C:\OpenServer\domains\dev-slando.com\resources\views/categories/pets/dogs.blade.php ENDPATH**/ ?>