<?php echo $__env->make('includes.user.head', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<?php echo $__env->make('includes.user.navbar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
<body>


<!-- ============== SECTION CONTENT ============== -->
<section class="padding-y">
    <div class="container">

        <?php $__currentLoopData = $advertisement; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if(auth()->guard()->check()): ?>
                <?php $statement = \App\Models\Favourite::where('advertisement_id', $item->id)
                        ->where('user_id', \Illuminate\Support\Facades\Auth::user()->getAuthIdentifier())
                        ->get()
                ?>
            <?php endif; ?>

            <div class="row">
            <aside class="col-lg-6">
                <article class="gallery-wrap">
                    <div class="img-wrap img-thumbnail">


                        <?php if(count($photos) > 0): ?>
                            <a data-fslightbox="mygalley" data-type="image" href="<?php echo e(asset($photos->first()['img_src'])); ?>">
                                <img height="260" src="<?php echo e(asset($photos->first()['img_src'])); ?>">
                            </a>
                        <?php elseif(count($photos) == 0): ?>
                            <img src="https://www.ctilogistics.com/wp-content/uploads/2012/10/500x3004.gif" alt="">
                        <?php endif; ?>

                    </div> <!-- img-big-wrap.// -->
                    <div class="thumbs-wrap">
                        <?php $__currentLoopData = $photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a data-fslightbox="mygalley" data-type="image" href="<?php echo e(asset($photo->img_src)); ?>" class="item-thumb">
                                <img width="60" height="60" src="<?php echo e(asset($photo->img_src)); ?>">
                            </a>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div> <!-- thumbs-wrap.// -->
                </article> <!-- gallery-wrap .end// -->

                <?php switch((int)$item->category->id):
                    case (9): ?>
                        <table class="table border table-hover mt-4">
                            <tr>
                                <th>Тип двигуна</th> <td> <?php echo e(ucfirst($item->engine_type)); ?> </td>
                            </tr>
                            <tr>
                                <th>Стан</th> <td> <?php echo e(ucfirst($item->type)); ?> </td>
                            </tr>
                            <tr>
                                <th>Коробка передач</th> <td> <?php echo e(ucfirst($item->transmission)); ?> </td>
                            </tr>
                            <tr>
                                <th>Тип приводу</th> <td> <?php echo e(ucfirst($item->type_of_drive)); ?> </td>
                            </tr>
                            <tr>
                                <th>Год выпуска</th> <td> <?php echo e(ucfirst($item->year)); ?> </td>
                            </tr>
                        </table>
                    <?php break; ?>;
                <?php endswitch; ?>

            </aside>
            <main class="col-lg-6">
                <article class="ps-lg-3">
                    <h4 class="title text-dark"><?php echo e($item->title); ?></h4>
                    <hr>

                    <div class="mb-3">
                        <var class="price h5">$<?php echo e(number_format($item->price, 0, ',', '.')); ?></var>
                        <span class="text-muted">/per box</span>
                    </div>

                    <p><?php echo e($item->description); ?></p>

                    <hr>

                    <?php if(auth()->guard()->check()): ?>
                        <?php if(count($statement) == 1): ?>
                            <a style="height: 40px" href="<?php echo e(route('favourite.delete', $item->id)); ?>" class="btn  btn-light">
                                <i style="color: red" class="me-1 fa fa-heart"></i> Видалити з обранного
                            </a>
                        <?php elseif(count($statement) == 0): ?>
                            <a style="height: 40px" href="<?php echo e(route('favourite.add', $item->id)); ?>" class="btn  btn-light">
                                <i class="me-1 fa fa-heart"></i> Додати до обранного
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>

                    <?php if(auth()->guard()->check()): ?>
                        <?php if(\Illuminate\Support\Facades\Auth::user()->id != $item->user->id && $item->category_id != 12 && $item->category_id != 13): ?>
                            <form class="mt-2" action="<?php echo e(route('charge')); ?>" method="post">
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name="id" value="<?php echo e($item['id']); ?>">
                                <input type="hidden" name="amount" value="<?php echo e($item['price']); ?>"/>

                                <button style="width: 200px; height: 40px; border-width: 1px; border-color: #dee2e6;" type="submit" name="submit" value="Pay Now" class="btn">
                                    <img height="25px" src="data:image/svg+xml;base64," data-v-b01da731="" alt="" role="presentation" class="paypal-logo paypal-logo-paypal paypal-logo-color-blue">
                                </button>
                            </form>
                        <?php endif; ?>
                    <?php endif; ?>

                </article> <!-- product-info-aside .// -->
            </main> <!-- col.// -->


        </div> <!-- row.// -->
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

    </div> <!-- container .//  -->
</section>
<!-- ============== SECTION CONTENT END// ============== -->



</body>


<?php /**PATH C:\OpenServer\domains\slando.com\resources\views/advertisement/showOne.blade.php ENDPATH**/ ?>